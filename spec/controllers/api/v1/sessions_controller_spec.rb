...
describe Api::V1::SessionsController , :type => :request do
  before do
    ...
    @invalid_params = { user: { email: 'test@test.com', password: 'pass1234'} }
  end
  ...
  context "POST Sessions" do
    context "failure" do
      ...
      it "with invalid user" do
        post api_v1_sessions_url(@params), headers: { "X-API-KEY": Rails.application.credentials.api[:app_key] }
      ...
      it "with invalid password" do
        ...
        post api_v1_sessions_url(@invalid_params), headers: { "X-API-KEY": Rails.application.credentials.api[:app_key] }
        expect(response).to have_http_status(:ok)
        body = JSON.parse(response.body)
        expect(body['success']).to be_falsey
        expect(body['data']).to be_nil
